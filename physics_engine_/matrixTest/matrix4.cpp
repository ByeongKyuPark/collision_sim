...
Matrix4 __stdcall Matrix4::operator*(const Matrix4& mat) const {
    Matrix4 temp{ transpose() };
    Matrix4 result{ 0 };

    __m128 c1 = _mm_load_ps(&temp.entries[0]);
    __m128 c2 = _mm_load_ps(&temp.entries[4]);
    __m128 c3 = _mm_load_ps(&temp.entries[8]);
    __m128 c4 = _mm_load_ps(&temp.entries[12]);
    for (int i = 0; i < 4; i++) {
        __m128 v1 = _mm_set1_ps(mat.entries[i]);
        __m128 v2 = _mm_set1_ps(mat.entries[i + 4]);
        __m128 v3 = _mm_set1_ps(mat.entries[i + 8]);
        __m128 v4 = _mm_set1_ps(mat.entries[i + 12]);
...
Matrix4::Matrix4(float i0, float i1, float i2, float i3, float i4, float i5, float i6, float i7, float i8, float i9, float i10, float i11, float i12, float i13, float i14, float i15)
...
Matrix4 Matrix4::inverse() const
{
    ...
    float determinant = temp.entries[0] * inv[0] + temp.entries[4] * inv[4] + temp.entries[8] * inv[8] + temp.entries[12] * inv[12];
